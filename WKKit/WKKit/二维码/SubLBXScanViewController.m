...
- (void)drawBottomItems
{
    ...
    _btnFlash.center = CGPointMake(CGRectGetWidth(_bottomItemsView.frame)*3/4-20, CGRectGetHeight(_bottomItemsView.frame)/2);
     [_btnFlash setImage:[UIImage imageNamed:@"CodeScan.bundle/qrcode_scan_btn_flash_nor"] forState:UIControlStateNormal];
    [_btnFlash addTarget:self action:@selector(openOrCloseFlash) forControlEvents:UIControlEventTouchUpInside];
    
    self.btnPhoto = [[UIButton alloc]init];
    _btnPhoto.bounds = _btnFlash.bounds;
    _btnPhoto.center = CGPointMake(CGRectGetWidth(_bottomItemsView.frame)/4+20, CGRectGetHeight(_bottomItemsView.frame)/2);
...
//    [self.navigationController pushViewController:vc animated:YES];
    ...
    UIAlertView *alert=[[UIAlertView alloc]initWithTitle:@"扫描结果" message: strResult.strScanned delegate:nil cancelButtonTitle:@"取消" otherButtonTitles:@"确定", nil];
    [alert show];
